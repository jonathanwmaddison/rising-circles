...
$( document ).ready(function() {
    ...
    //styling for buttons on mouseover
    $('.mobileButton').mouseover(function () {
            var color = $(this).css('background-color')
            var newColor = color.slice(0,color.length-1)+", 0.3"+color.slice(color.length-1)
            newColor = newColor.replace(/rgb/,'rgba')
            $(this).css('background-color', newColor)
        }).mouseout(function () {
           var buttonId = $(this).text()
           var buttonColor = keyData[buttonId].color
